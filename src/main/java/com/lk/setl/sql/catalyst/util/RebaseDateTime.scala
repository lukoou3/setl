...
object RebaseDateTime {
  ...
  private[sql] def loadRebaseRecords(fileName: String): AnyRefMap[String, RebaseInfo] = {
    ...
    val jsonRebaseRecords = mapper.readValue(file, new TypeReference[java.util.List[JsonRebaseRecordData]]{})
      .asScala.map(x => JsonRebaseRecord(x.tz, x.switches, x.diffs))
