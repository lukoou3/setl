...
/**
 * ExprCode表示一段Java代码，使用Row作为输入，计算一个Expression。
...
class CodegenContext extends Logging {
   ...
   *                 e.g. Row, UnsafeRow, UnsafeArrayData, etc. Other types will have to
  ...
  /**
   * Add buffer variable which stores data coming from an [[Row]]. This methods guarantees
   * that the variable is safely stored, which is important for (potentially) byte array backed
   * data types like: UTF8String, ArrayData, MapData & Row.
   */
  def addBufferedState(dataType: DataType, variableName: String, initCode: String): ExprCode = {
    ...
    val code = dataType match {
      ...
      case _: StructType | _: ArrayType => code"$value = $initCode.copy();"
  ...
  def genEqual(dataType: DataType, c1: String, c2: String): String = dataType match {
    ...
    case dt: DataType if isPrimitiveType(dt) => s"$c1 == $c2" // 这个必须是原始类型
  ...
  def genComp(dataType: DataType, c1: String, c2: String): String = dataType match {
    ...
    case schema: StructType =>
      ...
      val funcCode: String =
        s"""
          public int $compareFunc(Row a, Row b) {
    ...
    innerClassToFunctions.flatMap {
      case ((innerClassName, innerClassInstance), innerClassFunctions) =>
        ...
        if (orderedFunctions.size > MERGE_SPLIT_METHODS_THRESHOLD) {
          ...
          //     private void apply_862(Row i) { ... }
          //     private void apply_863(Row i) { ... }
          //       ...
          //     private void apply(Row i) {
          //       apply_862(i);
          //       apply_863(i);
          //       ...
          //     }
  ...
  def subexprFunctionsCode: String = {
    ...
    splitExpressions(subexprFunctions.toSeq, "subexprFunc_split", Seq("Row" -> INPUT_ROW))
  }
  ...
  private def subexpressionElimination(expressions: Seq[Expression]): Unit = {
    ...
    commonExprs.foreach { e =>
      ...
      val fn =
        s"""
           |private void $fnName(Row $INPUT_ROW) {
           |  ${eval.code}
           |  $isNull = ${eval.isNull};
           |  $value = ${eval.value};
           |}
...
abstract class CodeGenerator[InType <: AnyRef, OutType <: AnyRef] extends Logging {
  ...
  protected val genericMutableRowType: String = classOf[GenericRow].getName
...
object CodeGenerator extends Logging {
  ...
  private[this] def doCompile(code: CodeAndComment): (GeneratedClass, ByteCodeStats) = {
    ...
    evaluator.setClassName("com.lk.setl.sql.catalyst.expressions.GeneratedClass")
    // 默认导入的类
    ...
    .build(
      new CacheLoader[CodeAndComment, (GeneratedClass, ByteCodeStats)]() {
        override def load(code: CodeAndComment): (GeneratedClass, ByteCodeStats) = {
          ...
          val timeMs: Double = duration.toDouble / 1000000L // NANOS_PER_MILLIS
  ...
  def getValue(input: String, dataType: DataType, ordinal: String): String = {
    ...
    dataType match {
      ...
      case _ if isPrimitiveType(jt) => s"($bt)$input.get($ordinal)"
      case StringType => s"($jt)$input.get($ordinal)"
      case BinaryType => s"($jt)$input.get($ordinal)"
      case t: StructType => s"($jt)$input.get($ordinal)"
      case _: ArrayType => s"($jt)$input.get($ordinal)"
      case NullType => "null"
      case _ => s"($jt)$input.get($ordinal)"  // 这个没法转换基本数据类型把。
    }
  }

  /**
   * Generates code creating a [[UnsafeArrayData]] or
   * [[GenericArrayData]] based on given parameters.
   *
  ...
  def setColumn(row: String, dataType: DataType, ordinal: Int, value: String): String = {
    ...
    dataType match {
      ...
      case _ if isPrimitiveType(jt) => s"$row.update($ordinal, $value)"
      case StringType => s"$row.update($ordinal, $value)"
      // The UTF8String, Row, ArrayData and MapData may came from UnsafeRow, we should copy
  ...
  def getValueFromVector(vector: String, dataType: DataType, rowId: String): String = {
    if (dataType.isInstanceOf[StructType]) {
      // `ColumnVector.getStruct` is different from `Row.getStruct`, it only takes an
      // `ordinal` parameter.
  ...
  def getLocalInputVariableValues(
    ...
    if (ctx.INPUT_ROW != null) {
      argSet += JavaCode.variable(ctx.INPUT_ROW, classOf[Row])
    }
