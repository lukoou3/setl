...
import com.lk.setl.sql.catalyst.util.RebaseDateTime._
...
object DateTimeUtils {
  ...
  def cleanLegacyTimestampStr(s:String): String = s.replace(gmt, "")
   ...
   */
  def stringToTimestamp(s: String, timeZoneId: ZoneId): Option[Long] = {
    if (s == null) {
      return None
    }
    ...
    val bytes = s.trim.getBytes(StandardCharsets.UTF_8)
  ...
  def stringToTimestampAnsi(s: String, timeZoneId: ZoneId): Long = {
   ...
   * `yyyy-[m]m-[d]dT*`
   */
  def stringToDate(s: String, zoneId: ZoneId): Option[Int] = {
    if (s == null) {
      return None
    }
   ...
   */
  def getSecondsWithFraction(micros: Long, zoneId: ZoneId): Double = {
    getMicroseconds(micros, zoneId).toDouble / MICROS_PER_SECOND
  }
