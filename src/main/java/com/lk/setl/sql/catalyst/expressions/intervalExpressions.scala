...
case class ExtractIntervalMinutes(child: Expression)
  extends ExtractIntervalPart(child, IntegerType, getMinutes, "getMinutes")

case class ExtractIntervalSeconds(child: Expression)
  extends ExtractIntervalPart(child, DoubleType, getSeconds, "getSeconds")
...
case class MakeInterval(
  ...
  def this(
      ...
      mins: Expression,
      sec: Expression) = {
    this(years, months, weeks, days, hours, mins, sec, false)
  }
  def this(
      years: Expression,
      months: Expression,
      ...
      hours: Expression,
      mins: Expression) = {
    this(years, months, weeks, days, hours, mins, Literal(0D),
      false)
  }
  def this(
      years: Expression,
  ...
  override def inputTypes: Seq[AbstractDataType] = Seq(IntegerType, IntegerType, IntegerType,
    IntegerType, IntegerType, IntegerType, DoubleType)
  override def dataType: DataType = CalendarIntervalType
  ...
  override def nullSafeEval(
    ...
    try {
      IntervalUtils.makeInterval(
        ...
        sec.map(_.asInstanceOf[Double]).getOrElse(0D))
    } catch {
