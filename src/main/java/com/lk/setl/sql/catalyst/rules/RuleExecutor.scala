...
class PlanChangeLogger[TreeType <: TreeNode[_]] extends Logging {
  ...
  private val logLevel = "trace" // SQLConf.get.planChangeLogLevel

  private val logRules = Option.empty[Seq[String]] //SQLConf.get.planChangeRules.map(Utils.stringToSeq)

  private val logBatches = Option.empty[Seq[String]]// SQLConf.get.planChangeBatches.map(Utils.stringToSeq)
