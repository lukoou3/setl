...
import com.lk.setl.sql.catalyst.expressions.codegen.{CodeGenerator, CodegenContext, CodegenFallback, ExprCode}
import com.lk.setl.sql.catalyst.util.{DateTimeUtils, LegacyDateFormats, TimestampFormatter}
import com.lk.setl.sql.catalyst.util.DateTimeConstants._
import com.lk.setl.sql.catalyst.util.DateTimeUtils._
import com.lk.setl.sql.catalyst.util.LegacyDateFormats.SIMPLE_DATE_FORMAT
...
case class CurrentDate(timeZoneId: Option[String] = None)
  ...
  override def foldable: Boolean = false // 实时
  override def nullable: Boolean = false
   ...
   * 我们默认是Asia/Shanghai
   */
  override def eval(input: Row): Any = currentDate(zoneId)
...
abstract class CurrentTimestampLike() extends LeafExpression with CodegenFallback {
  ...
  override def eval(input: Row): Any = currentTimestamp()
}
...
case class CurrentBatchTimestamp(
   ...
   */
  override protected def evalInternal(input: Row): Any = toLiteral.value
...
case class DateFormatClass(left: Expression, right: Expression, timeZoneId: Option[String] = None)
  ...
  override protected def nullSafeEval(timestamp: Any, format: Any): Any = {
    ...
    formatter.format(timestamp.asInstanceOf[Long])
  }
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    formatterOption.map { tf =>
      ...
      defineCodeGen(ctx, ev, (timestamp, _) => {
        s"""$timestampFormatter.format($timestamp)"""
      })
    }.getOrElse {
...
case class ToUnixTimestamp(
  ...
  def this(timeExp: Expression, format: Expression) =
    this(timeExp, format, None, false)
      ...
      left.dataType match {
        ...
        case StringType =>
          ...
          if (fmt == null) {
            ...
            try {
              formatter.parse(t.asInstanceOf[String]) / downScaleFactor
            } catch {
...
case class FromUnixTime(sec: Expression, format: Expression, timeZoneId: Option[String] = None)
  ...
  override def nullSafeEval(seconds: Any, format: Any): Any = {
    ...
    fmt.format(seconds.asInstanceOf[Long] * MICROS_PER_SECOND)
  }
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    formatterOption.map { f =>
      ...
      defineCodeGen(ctx, ev, (seconds, _) =>
        s"$formatterName.format($seconds * 1000000L)")
    }.getOrElse {
      val tf = TimestampFormatter.getClass.getName.stripSuffix("$")
      val ldf = LegacyDateFormats.getClass.getName.stripSuffix("$")
      val zid = ctx.addReferenceObj("zoneId", zoneId, classOf[ZoneId].getName)
...
case class NextDay(startDate: Expression, dayOfWeek: Expression)
  ...
  override def nullSafeEval(start: Any, dayOfW: Any): Any = {
    ...
    val dow = DateTimeUtils.getDayOfWeekFromString(dayOfW.asInstanceOf[String])
    if (dow == -1) {
      null
    } else {
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    nullSafeCodeGen(ctx, ev, (sd, dowS) => {
      ...
      if (dayOfWeek.foldable) {
        val input = dayOfWeek.eval().asInstanceOf[String]
...
sealed trait UTCTimestamp extends BinaryExpression with ImplicitCastInputTypes with NullIntolerant {
  ...
  override def nullSafeEval(time: Any, timezone: Any): Any = {
    func(time.asInstanceOf[Long], timezone.asInstanceOf[String])
  }

  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    val dtu = DateTimeUtils.getClass.getName.stripSuffix("$")
    if (right.foldable) {
      val tz = right.eval().asInstanceOf[String]
      if (tz == null) {
        ev.copy(code = code"""
...
trait TruncInstant extends BinaryExpression with ImplicitCastInputTypes {
  ...
  private lazy val truncLevel: Int =
    DateTimeUtils.parseTruncLevel(format.eval().asInstanceOf[String])
   ...
   */
  protected def evalHelper(input: Row, minLevel: Int)(
...
case class MakeTimestamp(
  ...
  def this(
      ...
      min: Expression,
      sec: Expression) = {
    this(year, month, day, hour, min, sec, None, None, false)
  }
  ...
  def this(
      ...
      sec: Expression,
      timezone: Expression) = {
    this(year, month, day, hour, min, sec, Some(timezone), None, false)
  }
  ...
  private def toMicros(
      ...
      zoneId: ZoneId): Any = {
    try {
      val seconds = secAndMicros.toInt
      val unscaledSecFrac = (secAndMicros * MICROS_PER_SECOND).toLong
  ...
  override def nullSafeEval(
    ...
    val zid = timezone
      .map(tz => DateTimeUtils.getZoneId(tz.asInstanceOf[String].toString))
      .getOrElse(zoneId)
    toMicros(
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    nullSafeCodeGen(ctx, ev, (year, month, day, hour, min, secAndNanos, timezone) => {
      val zoneId = timezone.map(tz => s"$dtu.getZoneId($tz)").getOrElse(zid)
      s"""
      try {
        int seconds = (int)secAndNanos;
        int totalMicros = (int)(secAndNanos * 1000000);
        int nanos = totalMicros * 1000;
...
object DatePart {
  ...
  def toEquivalentExpr(field: Expression, source: Expression): Expression = {
    ...
    if (fieldEval == null) {
      ...
      val fieldStr = fieldEval.asInstanceOf[String].toString
