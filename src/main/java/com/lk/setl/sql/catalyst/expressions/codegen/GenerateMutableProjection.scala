...
import com.lk.setl.sql.catalyst.expressions.MutableProjection
...
/**
 * Generates byte code that produces a [[Row]] object that can update itself based on a new
 ...
 * The internal [[Row]] object created internally is used only when `target` is not used.
 */
    ...
    val codeBody = s"""
      ...
      class SpecificMutableProjection extends ${classOf[BaseMutableProjection].getName} {
        ...
        public SpecificMutableProjection(Object[] references) {
          ...
          mutableRow = new $genericMutableRowType(new Object[${expressions.size}]);
        ...
        public ${classOf[BaseMutableProjection].getName} target(Row row) {
          mutableRow = row;
          return this;
        }
