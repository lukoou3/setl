...
import com.lk.setl.sql.catalyst.util.LegacyDateFormats.{LegacyDateFormat, LENIENT_SIMPLE_DATE_FORMAT}
...
class MicrosCalendar(tz: TimeZone, digitsInFraction: Int)
  ...
  def getMicros(): Long = {
    ...
    d / MicrosCalendar.POW_10(digitsInFraction)
  }
  ...
  def setMicros(micros: Long): Unit = {
    val d = micros * MicrosCalendar.POW_10(digitsInFraction)
    fields(Calendar.MILLISECOND) = (d / MICROS_PER_SECOND).toInt
  }
}

object MicrosCalendar{
