...
import com.lk.setl.sql.catalyst.util.DateTimeUtils.millisToMicros
...
object IntervalUtils {
  ...
  def getMinutes(interval: CalendarInterval): Int = {
    ((interval.microseconds % MICROS_PER_HOUR) / MICROS_PER_MINUTE).toByte
  }

  def getSeconds(interval: CalendarInterval): Double = {
    (interval.microseconds % MICROS_PER_MINUTE).toDouble / MICROS_PER_SECOND
  }
  ...
  private val fallbackNotice = s"set LEGACY_FROM_DAYTIME_STRING to true " +
    ...
    while (i < bytes.length) {
      ...
      state match {
        ...
        case UNIT_BEGIN =>
          ...
          try {
            b match {
              case 'y' if s.indexOf(yearStr, i) == i =>
