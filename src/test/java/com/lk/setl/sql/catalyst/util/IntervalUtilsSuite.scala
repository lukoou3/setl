...
import com.lk.setl.sql.catalyst.util.IntervalUtils._
import com.lk.setl.sql.catalyst.util.IntervalUtils.IntervalUnit._
import com.lk.setl.sql.CalendarInterval
import com.lk.setl.sql.catalyst.util.IntervalUtils
import com.lk.setl.sql.catalyst.util.IntervalUtils.IntervalUnit.IntervalUnit
import org.scalatest.funsuite.AnyFunSuite

import java.util.concurrent.TimeUnit

class IntervalUtilsSuite extends AnyFunSuite {
  private def checkFromString(input: String, expected: CalendarInterval): Unit = {
    assert(stringToInterval(input) === expected)
    assert(safeStringToInterval(input) === expected)
  }

  private def checkFromInvalidString(input: String, errorMsg: String): Unit = {
    failFuncWithInvalidInput(input, errorMsg, s => stringToInterval(s))
    assert(safeStringToInterval(input) === null)
  }
  ...
  test("interval duration") {
    def duration(s: String, unit: TimeUnit, daysPerMonth: Int): Long = {
      IntervalUtils.getDuration(stringToInterval(s), unit, daysPerMonth)
    }
  ...
  test("negative interval") {
    def isNegative(s: String, daysPerMonth: Int): Boolean = {
      IntervalUtils.isNegative(stringToInterval(s), daysPerMonth)
    }
